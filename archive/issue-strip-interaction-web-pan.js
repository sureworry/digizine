  ...
  function setupZineStripInteraction(stripMount, options) {
    ...
    scrollerEl.addEventListener(
      ...
      function (e) {
        ...
        /* Desktop web: drag horizontally to pan the strip (trackpad still uses wheel below). */
        if (
          stripEl.classList.contains('zine-strip--open') &&
          e.pointerType === 'mouse'
        ) {
          stripEl.scrollLeft = startScrollLeft - (e.clientX - startX);
        }
      },
      true
    );
    ...
    stripEl.addEventListener(
      ...
      function (event) {
        ...
        /* Web: two-finger vertical swipe maps to strip scroll (existing); horizontal swipe uses deltaX. */
        if (Math.abs(deltaX) > Math.abs(deltaY)) {
          if (deltaX !== 0) {
            event.preventDefault();
            stripEl.scrollLeft += deltaX;
          }
    ...
    /*
     * Touch: axis lock — horizontal swipe drives strip scrollLeft; vertical passes through for page scroll.
     * (Wide “web” viewports too: touch monitors / iPad landscape where overflow-x auto isn’t used.)
     */
